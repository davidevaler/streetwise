...
const SegnalazioneData = require('../models/segnalazioneData');

// GET - Ottieni tutte le segnalazioni (per admin)
router.get('/', async (req, res) => {
    try {
        const segnalazioni = await SegnalazioneData.find().sort({ dataCreazione: -1 });
...
router.post('/', async (req, res) => {
    ...
    try {
        const nuovaSegnalazione = new SegnalazioneData({
            titolo: titolo.trim(),
...
router.put('/:id', async (req, res) => {
    ...
    try {
        const segnalazione = await SegnalazioneData.findByIdAndUpdate(
            req.params.id,
            { stato },
            { new: true }
        );
