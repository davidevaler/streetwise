...
// dobbiamo chiamare loadComunications in questo modo perché è una funzione async
(async () => {
  try {
    await loadCommunications();
  } catch (err) {
    showToast("Errore nel caricamneto delle comunicaioni", tipo='error');
    console.error(err);
  }
})();
...
async function loadCommunications() {     
  try {
    const response = await fetch(`/api/comunicazioni/admin/all`);
...
// Mostra allegati esistentiAdd commentMore actions
    ...
    if (comm.allegati && comm.allegati.length > 0) {
      existingDiv.innerHTML = '<strong>Allegati esistenti:</strong><br>' + 
        ...
        `<a href="/api/comunicazioni/admin/${commId}/allegato/${index}" target="_blank" style="margin-right: 10px;">${att.nomeFile}</a>`
        ).join('<br>');
...
async function handleCommunicationSubmit(e) {
  ...
  try {
    ...
    const url = currentEditingCommunicationId 
      ? `/api/comunicazioni/admin/${currentEditingCommunicationId}`
...
async function deleteCommunication(commId, title) {
  ...
  try {
    const response = await fetch(`/api/comunicazioni/admin/${commId}`, {
      method: 'DELETE'
    });
...
async function viewCommunication(commId) {     
  try {
    ...
    if (comm.allegati && comm.allegati.length > 0) {
      ...
      comm.allegati.map((att, index) => 
        `<a href="/api/comunicazioni/admin/${commId}/allegato/${index}" target="_blank" style="margin-right: 10px; color: #667eea;">${att.nomeFile}</a>`
      ).join('<br>');
    }
...
// Chiudi modal comunicazioni cliccando fuoriAdd commentMore actions
